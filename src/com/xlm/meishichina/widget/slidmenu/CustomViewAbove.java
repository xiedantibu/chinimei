...
import com.xlm.meishichina.widget.slidmenu.SlidingMenu.OnClosedListener;
import com.xlm.meishichina.widget.slidmenu.SlidingMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup
{
    ...
    private boolean thisSlideAllowed(float dx)
    {
        ...
        if (DEBUG)
            Logmeishi.v(TAG, "this slide allowed " + allowed + " dx: " + dx);
        return allowed;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev)
    {
        ...
        if (DEBUG)
            if (action == MotionEvent.ACTION_DOWN)
                Logmeishi.v(TAG, "Received ACTION_DOWN");
        ...
        switch (action)
        {
            case MotionEvent.ACTION_MOVE:
                ...
                try
                {
                    ...
                    if (DEBUG)
                        Logmeishi.v(TAG, "onInterceptTouch moved to:(" + x
                                + ", " + y + "), diff:(" + xDiff + ", " + yDiff
        ...
        switch (action & MotionEventCompat.ACTION_MASK)
        {
            ...
            case MotionEvent.ACTION_MOVE:
                if (!mIsBeingDragged)
                {
                    ...
                    final float yDiff = Math.abs(y - mLastMotionY);
                    if (DEBUG)
                        Logmeishi.v(TAG, "onTouch moved to:(" + x + ", " + y
                                + "), diff:(" + xDiff + ", " + yDiff
                    ...
                    {
                        if (DEBUG)
                            Logmeishi.v(TAG, "Starting drag! from onTouch");
                        startDrag();
                        mLastMotionX = x;
                        setScrollingCacheEnabled(true);
                    }
                    ...
                    {
                        if (DEBUG)
                            Logmeishi.v(TAG, "onTouch returning false");
                        return false;
                    }
                }
                if (mIsBeingDragged)
    ...
    {
        if (DEBUG)
            Logmeishi.v(TAG, "onSecondaryPointerUp called");
